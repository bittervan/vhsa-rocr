...
hsa_status_t AMD::VhsaDriver::DiscoverDriver(std::unique_ptr<core::Driver>& driver) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        {
            // 找到了我们的 vHSA 设备！
            rocr::os::PrintError("vHSA: Found vHSA PCI device at %s\n", entry->d_name);
            closedir(dir);
            ...
            if (tmp_driver->Open() == HSA_STATUS_SUCCESS) {
                ...
                return HSA_STATUS_ERROR_DRIVER_NOT_INITIALIZED;
            }
        }
    }
...
{
    rocr::os::Printerr("vHSA: VhsaDriver constructor called for device path: %s\n", devnode_name.c_str());
}
...
hsa_status_t VhsaDriver::Open() {
    ...
    rocr::os::Printerr("vHSA: Open() called. Simulating mmap to QEMU device...\n");
    is_open_ = true; // 假设成功
    return HSA_STATUS_SUCCESS;
}
...
hsa_status_t VhsaDriver::Close() {
    ...
    rocr::os::Printerr("vHSA: Close() called. Simulating munmap...\n");
...
hsa_status_t VhsaDriver::Init() {
    ...
    rocr::os::Printerr("vHSA: Driver Init() called.\n");
    ...
    rocr::os::Printerr("vHSA: Simulating agent discovery...\n");
...
hsa_status_t VhsaDriver::ShutDown() {
    rocr::os::Printerr("vHSA: ShutDown() called.\n");
    return HSA_STATUS_SUCCESS;
}
...
hsa_status_t VhsaDriver::QueryKernelModeDriver(core::DriverQuery query) {
    rocr::os::Printerr("vHSA: QueryKernelModeDriver() NOT IMPLEMENTED.\n");
    // 我们可以伪造一个版本号
...
hsa_status_t VhsaDriver::GetSystemProperties(HsaSystemProperties& sys_props) const {
    rocr::os::Printerr("vHSA: GetSystemProperties() NOT IMPLEMENTED.\n");
...
hsa_status_t VhsaDriver::GetNodeProperties(HsaNodeProperties& node_props, uint32_t node_id) const {
    rocr::os::Printerr("vHSA: GetNodeProperties() NOT IMPLEMENTED.\n");
    // TODO (vHSA): 从宿主机获取并转发真实信息
    return HSA_STATUS_ERROR_NOT_SUPPORTED;
}
...
hsa_status_t VhsaDriver::AllocateMemory(const core::MemoryRegion &mem_region,
                              ...
                              uint32_t node_id) {
    rocr::os::Printerr("vHSA: AllocateMemory() NOT IMPLEMENTED.\n");
...
hsa_status_t VhsaDriver::GetEdgeProperties(std::vector<HsaIoLinkProperties>&, uint32_t) const { return HSA_STATUS_ERROR_NOT_SUPPORTED; }
hsa_status_t VhsaDriver::GetAgentProperties(core::Agent&) const { return HSA_STATUS_ERROR_NOT_SUPPORTED; }
hsa_status_t VhsaDriver::GetMemoryProperties(uint32_t, std::vector<HsaMemoryProperties>&) const { return HSA_STATUS_ERROR_NOT_SUPPORTED; }
hsa_status_t VhsaDriver::GetCacheProperties(uint32_t, uint32_t, std::vector<HsaCacheProperties>&) const { return HSA_STATUS_ERROR_NOT_SUPPORTED; }
hsa_status_t VhsaDriver::ExportDMABuf(void*, size_t, int*, size_t*) { return HSA_STATUS_ERROR_NOT_SUPPORTED; }
hsa_status_t VhsaDriver::ImportDMABuf(int, core::Agent&, core::ShareableHandle&) { return HSA_STATUS_ERROR_NOT_SUPPORTED; }
hsa_status_t VhsaDriver::Map(core::ShareableHandle, void*, size_t, size_t, hsa_access_permission_t) { return HSA_STATUS_ERROR_NOT_SUPPORTED; }
hsa_status_t VhsaDriver::Unmap(core::ShareableHandle, void*, size_t, size_t) { return HSA_STATUS_ERROR_NOT_SUPPORTED; }
hsa_status_t VhsaDriver::ReleaseShareableHandle(core::ShareableHandle&) { return HSA_STATUS_ERROR_NOT_SUPPORTED; }
hsa_status_t VhsaDriver::SPMAcquire(uint32_t) const { return HSA_STATUS_ERROR_NOT_SUPPORTED; }
hsa_status_t VhsaDriver::SPMRelease(uint32_t) const { return HSA_STATUS_ERROR_NOT_SUPPORTED; }
hsa_status_t VhsaDriver::SPMSetDestBuffer(uint32_t, uint32_t, uint32_t*, uint32_t*, void*, bool*) const { return HSA_STATUS_ERROR_NOT_SUPPORTED; }
